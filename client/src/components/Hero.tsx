        ...
        <div className="container mx-auto px-4 md:px-6">
          <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-white/20">
            {[
              ...
              { icon: DollarSign, title: "Revenue Cycle Management" },
              { icon: Users, title: "Practice Management" },
              { icon: UserCircle, title: "Patient Portal" },
            ].map((item, i) => (
              <div key={i} className="flex items-center gap-3 py-5 px-4 md:px-6 justify-center" data-testid={`banner-solution-${i}`}>
                ...
                <p className="text-white font-bold text-sm whitespace-nowrap">{item.title}</p>
              </div>
            ))}
          </div>
        </div>
